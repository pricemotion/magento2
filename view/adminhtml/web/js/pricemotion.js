function Pricemotion(rootSelector) {
  ...
  window.addEventListener('message', function (e) {
    ...
    if (message.type === 'setWidgetHeight') {
      ...
      validatedResolvers.forEach(([resolve, reject]) => (message.isValid ? resolve() : reject()));
      validatedResolvers = [];
    }
  });
  ...
  document.body.addEventListener('click', function (e) {
    if (e.target.closest('.pricemotion-submit')) {
      e.preventDefault();
      const valid = new Promise((resolve, reject) => validatedResolvers.push([resolve, reject]));
      valid.then(() => form.submit());
      valid.catch(() => console.error('invalid'));
      frame.contentWindow.postMessage({ type: 'validate' }, settings.web_origin);
    }
  });
  ...
  document.body.addEventListener(
    'click',
    function (e) {
      if (e.target.closest('.save') && !e.target.closest('.pricemotion-submit') && isValid === false) {
